...
def _clear_cache() -> None:
    ...
    __pil_cache.clear()
    __img_stats.clear()
    threading.Thread(target = gc.collect, daemon=False).start()
...
def efface(objet_ou_tag: Union[int, str]) -> None:
    ...
    if isinstance(objet_ou_tag, str):
        objets = __canevas.canvas.find_withtag(objet_ou_tag)
        for objet in objets:
            __img_stats.pop(objet, None)
    if isinstance(objet_ou_tag, int):
        if objet_ou_tag in __img_stats:
            __img_stats.pop(objet_ou_tag, None)
