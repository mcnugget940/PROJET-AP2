...
def dessiner_personnage(personnage):
    """Dessine le personnage sous forme d'un petit mouton simplifié."""
    px, py = personnage["position"]
    # Corps blanc
    fltk.rectangle(px, py, px + LARGEUR, py + HAUTEUR,
                   couleur='#444444', remplissage='white')
    # Yeux
    fltk.rectangle(px + 3, py + 5, px + 8, py + 10,
                   couleur='black', remplissage='black')
    fltk.rectangle(px + 12, py + 5, px + 17, py + 10,
                   couleur='black', remplissage='black')
...
def dessiner_jeu(personnage, lst_blocs, objectif, souris, nb_sauts):
    """Redessine l'intégralité de la scène de jeu."""
    fltk.efface_tout()

    # Fond
    fltk.rectangle(0, 0, LARGEUR_FENETRE, HAUTEUR_FENETRE,
                   couleur='#dce8dc', remplissage='#dce8dc')
...
def simuler_et_afficher(personnage, lst_blocs, objectif, nb_sauts):
    ...
    Renvoie True si la simulation s'est terminée normalement,
    False si une boucle infinie a été détectée (le coup sera annulé).
    """
    ...
    for i in range(MAX_PAS_SIMULATION):
        ...
        # Mettre à jour l'affichage tous les 2 pas
        if i % 2 == 0:
            dessiner_jeu(personnage, lst_blocs, objectif, None, nb_sauts)
...
def boucle_jeu(nom_fichier):
    ...
    dessiner_jeu(personnage, lst_blocs, objectif, souris, nb_sauts)
    ...
    while True:
        ...
        if te == 'Quitte':
            ...
            ok = simuler_et_afficher(personnage, lst_blocs, objectif, nb_sauts)
